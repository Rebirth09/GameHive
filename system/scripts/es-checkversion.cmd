...
es-checkversion.cmd
---------------------------------------
This Batch script is originally created for GameHive and to be used by the Windows build of Batocera-EmulationStation.
It exists in conjunction with other scripts to form an integrated update system within the EmulationStation interface.
Its main task is to check the local version of GameHive and compare it to the latest remote version available.
...
set "reg_path=HKCU\Software\GameHive"
set "reg_key=LatestKnownInstallPath"

reg query "HKCU\Software\GameHive" /v "%reg_key%" >nul 2>&1
